...
def main(context):
    ...
    seasonStats_collection_id = os.environ['SEASON_STATS_COLLECTION_ID']
    ...
    # Retrieve season stats
        season_data = get_season_stats(leagueID=allSeasonID[i])
    
    
        if season_data and 'data' in season_data:
    
            seasonDataDF = convert_json_to_df(season_data['data'])
            seasonDataJSON=prepare_for_appwrite(seasonDataDF)
    
            docIDs = get_all_document_ids(database_id, seasonStats_collection_id)
            classifications=group_columns(season_data['data'])
            ...
            try:
                ...
                attr_categories = get_categorized_attributes(database_id, seasonStats_collection_id)
            ...
            for season in seasonDataJSON:
                try:
                    if(season['id'] in docIDs):
                        docUpdate=databases.update_document(
                            database_id=database_id,
                            collection_id=seasonStats_collection_id,
                            document_id=allSeasonNames[i],
                            data=season
                        )
