...
def main(context):
    ...
    match_details_collection_id = os.environ['MATCH_DETAILS_COLLECTION_ID']
    ...
    upcoming_matches = get_upcoming_matches(0)
    
    if upcoming_matches and 'data' in upcoming_matches:
        matches=upcoming_matches['data']
        matchID=[]
        ...
        def create_collection_attributes(classifications, database_id: str, collection_id: str):
            ...
            try:
                ...
                for attr in id_attribute:
                    ...
                    size=500000  # Adjust size based on your ID format
                    )
                ...
                for attr in string_attributes:
                    databases.create_string_attribute(
                        ...
                        size=500000  # Adjust size as needed
                    )
        ...
        for i in range(0,count):
            ...
            match_details = get_matchDetails(matchID[i])
            if match_details and 'data' in match_details:
                matchList.append(match_details['data'])
    
    
        matchDetailsDataDF = convert_json_to_df(matchList)
        matchDetailsDataJSON=prepare_for_appwrite(matchDetailsDataDF)
    
        docIDs = get_all_document_ids(database_id, match_details_collection_id)
        classifications=group_columns(matchList)
     
        try:
            attList = databases.list_attributes(
            database_id = database_id,
            collection_id= match_details_collection_id
            )
    
            attr_categories = get_categorized_attributes(database_id, match_details_collection_id)
            ...
            if (attList['total'] == 0):
                ...
                context.log('Initial Attributes added for '+ match_details_collection_id)
            ...
            context.log('Check attributes for '+ str(matchID[i]))
    
        for matchDetails in matchDetailsDataJSON:
    
            try:
                if(matchDetails['id'] in docIDs):
    
                    docUpdate=databases.update_document(
                        database_id=database_id,
                        collection_id=match_details_collection_id,
                        document_id=matchDetails['id'],
                        data=matchDetails
                    )
    
                    context.log(matchDetails['id']+' Updated')
                    ...
                    try:
                        ...
                        context.log('Documents created for '+matchDetails['id'])
                    except Exception as e:
                        ...
                        if field_name and field_name in matchDetails:
                            context.log(f"\nProblem field details:")
                            context.log(f"{field_name} type: {type(matchDetails[field_name])}")
                            context.log(f"{field_name} length: {len(str(matchDetails[field_name]))}")
                            context.log(f"Preview: {str(matchDetails[field_name])[:100]}...")
            except:
                context.log('Check document '+ str(matchDetails['id']))
